...
const foodUnitSchema = new mongoose.Schema(
  { label: { type: String, required: true }, grams: { type: Number, required: true, min: 0 } },
  { _id: false }
);
...
// Todas as categorias servem café/almoço/janta/lanche; Bebidas serve só o tipo "drink".
function mealTypesFor(category) {
  return category === "Bebidas" ? ["drink"] : ["breakfast", "lunch", "dinner", "snack"];
}
...
async function main() {
  ...
  console.log(`Conectado. Semeando ${RAW_FOODS.length} alimentos...`);
  ...
  for (const raw of RAW_FOODS) {
    const doc = {
      ...
      mealTypes: mealTypesFor(raw.category),
      kcalPer100g: raw.kcalPer100g,
      carbPer100gG: raw.carbPer100gG,
      proteinPer100gG: raw.proteinPer100gG,
      fatPer100gG: raw.fatPer100gG,
      units: raw.units ?? [],
    };
    const result = await Food.updateOne({ slug: doc.slug }, { $set: doc }, { upsert: true });
